...
func ParsePortRange(portRange string) (int, int, error) {
    ...
    if strings.Contains(portRange, "-") {
        ...
        if err1 != nil || err2 != nil {
            return 0, 0, fmt.Errorf("invalid port numbers")
        }

        if start < 1 || end > 65535 || start > end {
            return 0, 0, fmt.Errorf("port range must be between 1-65535 and start <= end")
        }
    ...
    if port < 1 || port > 65535 {
        return 0, 0, fmt.Errorf("port must be between 1-65535")
    }
...
func ValidateConfig(config *models.ScanConfig) error {
    ...
    if config.StartPort < 1 || config.EndPort > 65535 {
        return fmt.Errorf("ports must be between 1-65535")
    }
