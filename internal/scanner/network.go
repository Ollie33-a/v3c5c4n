...
func (ns *NetworkScanner) Scan() (*models.HostResult, error) {
    ...
    if len(result.FilteredPorts) > 0 && !ns.config.ScanFiltered {
        ns.logger.Warn("Found %d filtered ports. Attempting filtered port scan...", len(result.FilteredPorts))
        response := ns.promptFilteredScan()
        if response {
            ns.config.ScanFiltered = true
            filteredResults := ns.scanFiltered(result.FilteredPorts)
            result.OpenFilteredPorts = append(result.OpenFilteredPorts, filteredResults...)
        }
    }
...
func (ns *NetworkScanner) scanTCP() *TCPScanResult {
    ...
    results := make(chan models.PortResult, ns.config.TotalPorts)
...
// scanFiltered attempts to scan filtered ports using evasion
func (ns *NetworkScanner) scanFiltered(filteredPorts []models.PortResult) []models.PortResult {
    result := make([]models.PortResult, 0)

    ns.logger.Info("Attempting to scan %d filtered ports using evasion techniques...", len(filteredPorts))
    ...
    for _, port := range filteredPorts {
        ...
        for _, method := range ns.config.EvasionMethods {
            ns.evasionEngine.ApplyEvasion(method)
            
            // Retry scan with evasion
            retryResult := ns.scanTCPPort(port.Port)
            
            if retryResult.State == "open" {
                retryResult.EvasionMethod = method
                result = append(result, retryResult)
                ns.logger.Success("Port %d found open using %s evasion", port.Port, method)
                break
            }
        }
    }
...
// promptFilteredScan prompts user to scan filtered ports
func (ns *NetworkScanner) promptFilteredScan() bool {
    // In real implementation, prompt user via stdin
